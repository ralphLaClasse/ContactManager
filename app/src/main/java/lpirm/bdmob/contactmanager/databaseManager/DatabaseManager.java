package lpirm.bdmob.contactmanager.databaseManager;
...
public class DatabaseManager implements DatabaseConstants {
	...
	private class CustomSQLiteOpenHelper extends SQLiteOpenHelper {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			String DATABASE_CREATE_MORE_TABLE = "create table " + "more_table"
					+ " (" + "contact_test" + " text not null" + ");";

			switch (oldVersion) {
			case 1:
				// no action for version 1
			case 2:
				db.execSQL(DATABASE_CREATE_MORE_TABLE);
	...
	public ArrayList<ContactModel> getAllDataAlternative() {
		...
		try {
			...
			/*
			 * queryStatement = "SELECT " + TABLE_ROW_NAME + "," +
			 * TABLE_ROW_PHONENUM + "," + TABLE_ROW_EMAIL + "," +
			 * TABLE_ROW_PHOTOID + " FROM " + TABLE_NAME;
			 */
